...
describe('public source contracts', () => {
  it.each(sourcePageCases)(
    '$name keeps source entries page-local and bounded',
    ({ directSourceEntries, furtherReadingEntries, name }) => {
  ...
  it.each(practitionerWaysCases)(
    ...
    ({ practitionerPath, waysImport, waysSidecarPath, name }) => {
      const content = read(practitionerPath);
