...
A fifth scan (Nuclei) runs automatically after seed and preview deployments, and it can also be triggered manually for any target URL. Results appear in the workflow run logs. SARIF upload to the Security tab is currently disabled in this repo setup.
