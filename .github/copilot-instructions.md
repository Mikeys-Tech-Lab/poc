...
- Durable learning: use `Trace, Reflect and Evolve` for non-trivial post-task reflection when the work exposed a reusable lesson. Preserve substantive origin prompts and material activation steps, not raw chat logs.
- Documentation evolution: every PR updates all affected documentation in the same PR. No deferred doc updates.
- Onboarding: structured docs at `docs/onboarding/` for repo-native work. Use `onboard me` for setup and workspace orientation. Use `Evolution Arc` for repo history and reasoning trace. Use `Trace, Reflect and Evolve` for durable learning capture after non-trivial work. Guide newcomers by reading the topic index at `docs/onboarding/README.md`.
